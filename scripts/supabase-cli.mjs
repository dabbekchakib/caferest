...
const needsProjectRef = args.includes("link") || args.includes("push") || args.includes("reset") || args.includes("status") || args.includes("pull");
const needsToken = args.includes("link") || args.includes("push") || args.includes("reset") || args.includes("pull");
const needsDbPassword = args.includes("push") || args.includes("reset");

if (needsToken && !vars.SUPABASE_ACCESS_TOKEN) {
  console.error("✖ SUPABASE_ACCESS_TOKEN is missing. Create one in Supabase Dashboard → Account → Access Tokens and add it to .env.local");
  process.exit(1);
}
if (needsProjectRef && !vars.SUPABASE_PROJECT_REF) {
  console.error("✖ SUPABASE_PROJECT_REF is missing. Set it in .env.local (the subdomain of your project URL).");
  process.exit(1);
}
if (needsDbPassword && !vars.SUPABASE_DATABASE_PASSWORD) {
  console.error("✖ SUPABASE_DATABASE_PASSWORD is missing. Set the PostgreSQL 'postgres' role password in .env.local.");
  process.exit(1);
}

// Non-interactive: pass --password when available so we don't depend on a TTY prompt.
if (args[0] === "link" && !args.includes("--password") && !args.some((a) => a === "-p") && vars.SUPABASE_DATABASE_PASSWORD) {
  args.push("--password", vars.SUPABASE_DATABASE_PASSWORD);
}

// link requires the project ref as a flag/arg; inject it from the env.
if (args[0] === "link" && vars.SUPABASE_PROJECT_REF && !args.includes("--project-ref")) {
