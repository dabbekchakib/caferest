...
interface ProductSelectorProps {
  ...
  limit?: number;
  placeholder?: string;
  onSelect: (product: ProductSelectorEntry) => void;
  disabled?: boolean;
}

/** Debounced product search with an inline results dropdown. */
export function ProductSelector({
  ...
  const [results, setResults] = useState<ProductSelectorEntry[]>([]);
  ...
  useEffect(() => {
    ...
    timer.current = setTimeout(
      async () => {
        ...
        setBusy(true);
        const result = await searchProductsAction({ query: q, locale, limit });
        setBusy(false);
        if (result.ok) {
